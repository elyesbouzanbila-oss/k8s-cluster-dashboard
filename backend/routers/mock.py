...
async def get_pods_mock(settings: Settings = Depends(verify_api_key)):
    """Return mock pods"""
    return {"items": MOCK_PODS}

@router.get("/mock/topology")
async def get_topology_mock(settings: Settings = Depends(verify_api_key)):
    """Return mock topology with cluster nodes, pods, and services"""
    return build_mock_topology()

@router.get("/mock/rbac")
async def get_rbac_mock(settings: Settings = Depends(verify_api_key)):
    """Return mock RBAC"""
    return MOCK_RBAC

@router.get("/mock/privileged")
async def get_privileged_mock(settings: Settings = Depends(verify_api_key)):
